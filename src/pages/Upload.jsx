  ...
  const saveAllToServer = async () => {
    ...
    try {
      ...
      if (withFile.length === 0) {
        toast.error(
          "Không có bản ghi nào có file audio hợp lệ. Kiểm tra MusicUploader có trả về 'file' chưa."
        );
        return;
      }
      ...
      for (const t of withFile) {
        try {
          ...
          toast.error(`Upload thất bại: ${t.name}`);
        }
      }

      if (results.length === 0) {
        toast.error(
          "Không upload được bản nào. Kiểm tra Console hoặc Network tab để xem chi tiết lỗi."
        );
        return;
      }
      ...
      toast.success(
        `Đã upload ${results.length}/${withFile.length} bài lên server!`
      );
    } finally {
      setSaving(false);
    }
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">
            Upload multiple music files at once and manage your library
          </p>
        </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) =>
                    setEditForm({ ...editForm, keepFile: e.target.checked })
                  }
                  className="w-5 h-5 rounded border-gray-600"
                />
                <label htmlFor="keepFile" className="text-sm">
                  Keep old music file (only update information)
                </label>
              </div>
              {!editForm.keepFile && (
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (f) {
                        ...
                        setTracks((prev) =>
                          prev.map((t) =>
                            t.id === editingTrack.id ? { ...t, file: f } : t
                          )
                        );
                      }
                    }}
